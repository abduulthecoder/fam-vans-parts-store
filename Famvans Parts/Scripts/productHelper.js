 ...
 */
export function calculateJobPrice(retailPrice, laborHours, laborRate = 225) {
...
export function getCompatibleProducts(vans, inventory, vanSpecs) {
  // Find matching van(s)
  const matchingVans = findVansBySpecs(vans, vanSpecs);
  if (matchingVans.length === 0) return [];

  // Build comprehensive search identifiers from all matching vans
  const searchIdentifiers = new Set();

  matchingVans.forEach(van => {
    // Add individual components
    searchIdentifiers.add(van.year.toString());
    searchIdentifiers.add(van.make);
    searchIdentifiers.add(van.model);
    searchIdentifiers.add(van.type);

    // Add common identifiers
    searchIdentifiers.add(`${van.year}`);
    searchIdentifiers.add(`${van.make} ${van.model}`);
    searchIdentifiers.add(`${van.year} ${van.make}`);

    // Extract key model components (e.g., "Transit" from "Transit 150 Cargo Van Low Roof")
    const modelParts = van.model.split(' ');
    modelParts.forEach(part => {
      if (part && part.length > 2) searchIdentifiers.add(part);
    });

    // Extract roof type if present
    if (van.roof) searchIdentifiers.add(van.roof);
  });
  ...
  for (const category in inventory) {
    inventory[category].forEach(product => {
      ...
      // Check if any search identifier matches in the fitment string
      let matches = false;
      for (const id of searchIdentifiers) {
        const searchTerm = id.toLowerCase();
        if (fitment.includes(searchTerm)) {
          matches = true;
          break;
        }
      }
