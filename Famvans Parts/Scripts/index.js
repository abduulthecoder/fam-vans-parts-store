...
// ==================== CART UTILITIES ====================
function addToCart(product, qty = 1, laborHours = 0) {
  const cart = JSON.parse(localStorage.getItem('cart') || '[]');
  // find same product & labor combination
  const existingIdx = cart.findIndex(item => item.id === product.part_number && item.laborHours === laborHours);
  if (existingIdx > -1) {
    cart[existingIdx].qty += qty;
  } else {
    cart.push({
      id: product.part_number,
      title: product.part_description || product.name || 'Product',
...
// Van Selector Elements (will be assigned after DOM ready)
let yearSelect = null;
let makeSelect = null;
...
async function loadVansDatabase() {
  try {
    ...
    console.log('loadVansDatabase: loaded vans count=', vansDatabase.length);
    populateYears();
  } catch (error) {
...
function populateYears() {
  if (!yearSelect) {
    console.warn('populateYears: yearSelect element not found');
    return;
  }
  const years = getAvailableYears(vansDatabase || []);
  console.log('populateYears: years=', years.slice(0,10));
...
function populateMakes() {
  ...
  const makes = !selectedYear
    ...
    : getMakesForYear(vansDatabase, selectedYear);
...
function populateModels() {
  ...
  models.forEach(({ model, modelNumber }) => {
    const option = document.createElement('option');
    option.value = model; // Use full model name for better matching
...
function displayVanInfo(year, make, model) {
  ...
  vanInfo.id = 'van-info';
  vanInfo.style.cssText = `background:#f0f8ff;border:1px solid #007bff;border-radius:5px;padding:10px;margin-bottom:20px;text-align:center;font-weight:bold;`;
  ...
  if (container) container.insertBefore(vanInfo, container.firstChild);
}
...
// Initialize on page load and wire up selectors only when present
document.addEventListener('DOMContentLoaded', () => {
  // Assign selector elements (may be null on product pages)
  yearSelect = document.getElementById('year-select');
  makeSelect = document.getElementById('make-select');
  modelSelect = document.getElementById('model-select');
  searchBtn = document.getElementById('search-btn');

  // Wire up home page selector listeners if elements exist
  if (yearSelect) yearSelect.addEventListener('change', () => { populateMakes(); populateModels(); });
  if (makeSelect) makeSelect.addEventListener('change', populateModels);
  ...
  [yearSelect, makeSelect, modelSelect].forEach(select => {
    if (select) select.addEventListener('keypress', function(e) {
...
function displayProducts() {
  ...
  paginatedProducts.forEach(product => {
    ...
    // attach add to cart on newly created card
    const addBtn = card.querySelector('.add-to-cart');
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        const labor = parseFloat(prompt('Enter labor hours (0 for none):', product.labor_hours || 0)) || 0;
        addToCart(product, 1, labor);
      });
    }
  });
...
function openQuickView(e) {
  ...
  // attach listener for modal add-to-cart
  const modalBtn = document.querySelector('.add-to-cart-modal');
  if (modalBtn) {
